...
export const TransferDialogContent: React.FC<TransferDialogContentProps> = ({
  ...
  if (showSuccess) {
    return (
      <div
        className={`p-12 text-center space-y-6 ${isDark ? "bg-[#0d1017]" : "bg-white"}`}
      >
  ...
  return (
    <div
      className={`flex flex-col transition-colors duration-300 ${isDark ? "bg-[#0d1017] text-slate-200" : "bg-white text-slate-900"}`}
    >
      {/* Header Section */}
      <div
        className={`p-8 pb-10 border-b bg-linear-to-br from-indigo-500/10 via-transparent to-transparent ${isDark ? "border-white/5" : "border-slate-100"}`}
      >
        <div className="space-y-2">
          <div className="flex items-center gap-3">
            <div className="w-1.5 h-8 rounded-full bg-indigo-500 shadow-lg shadow-indigo-500/40" />
            <h2
          ...
          <p
            className={`text-[10px] font-black uppercase tracking-[0.25em] ml-4 ${isDark ? "text-slate-500" : "text-slate-400"}`}
          >
      ...
      <div className="p-8 space-y-8">
        ...
        <div className="space-y-3">
          <label
            className={`text-[10px] font-black uppercase tracking-[0.2em] block ml-1 ${isDark ? "text-slate-500" : "text-slate-400"}`}
          >
          ...
          <div className={`relative group`}>
            ...
            <input
              ...
              className={`w-full py-5 pl-14 pr-6 rounded-2xl border font-mono text-sm transition-all focus:ring-0 outline-none ${
                isDark
                  ? "bg-white/2 border-white/5 focus:border-indigo-500/50 text-white placeholder:text-slate-700"
                  : "bg-slate-50 border-slate-200 focus:border-indigo-500 text-slate-900 placeholder:text-slate-300"
              }`}
            />
          </div>
        </div>
        ...
        >
          ...
          <div className="space-y-2">
            <h4
              className={`text-xs font-black uppercase tracking-tight ${isDark ? "text-slate-100" : "text-slate-900"}`}
            >
              Critical Ownership Action
            </h4>
            <p
              className={`text-[11px] leading-relaxed font-medium ${isDark ? "text-slate-500" : "text-slate-600"}`}
            >
        ...
        <div className="flex gap-4 pt-2">
          <button
            onClick={onClose}
            className={`flex-1 py-5 rounded-2xl font-black uppercase tracking-[0.25em] text-[10px] transition-all border ${isDark ? "text-slate-500 hover:text-white hover:bg-white/5 border-white/5" : "text-slate-400 hover:text-slate-900 hover:bg-slate-100 border-slate-100"}`}
          >
            Cancel
          </button>
          <button
